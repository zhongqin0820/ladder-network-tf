...
def train(arg):
    ...
    if arg == 'MNIST':
        INPUT_SIZE = 784
        layer_sizes = [INPUT_SIZE, 1000, 500, 250, 250, 250, 10]
        ...
        print('python ladder.py -t <[MNIST,CIFAR10]>')
        sys.exit(2)
    ...
    print("===  Loading Data ===")
    if arg == 'MNIST':
        dataset = input_data.read_data_sets("MNIST_data", n_labeled=num_labeled, one_hot=True)
        ckpt = tf.train.get_checkpoint_state('checkpoints/')  # get latest checkpoint (if any)
    else:
        dataset = input_data.read_cifar10("CIFAR10_data", n_labeled=num_labeled, one_hot=True)
        ckpt = tf.train.get_checkpoint_state('checkpoints_cifar/')  # get latest checkpoint (if any)
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        if arg == 'MNIST':
            ...
            # no checkpoint exists. create checkpoints directory if it does not exist.
            if not os.path.exists('checkpoints_cifar'):
                os.makedirs('checkpoints_cifar')
        init = tf.global_variables_initializer()
        sess.run(init)

    print("=== Training ===")
    print("Initial Accuracy: ", sess.run(accuracy, feed_dict={inputs: dataset.test.images, outputs: dataset.test.labels, training: False}), "%")
    ...
    for i in tqdm(range(int(i_iter), int(num_iter))):
        ...
        if (i > 1) and ((i+1) % (num_iter/num_epochs) == 0):
            ...
            # pdb.set_trace()
            # print "Epoch ", epoch_n, ", Accuracy: ", sess.run(accuracy, feed_dict={inputs: dataset.test.images, outputs:dataset.test.labels, training: False}), "%"
            with open('train_log', 'a+', newline='') as train_log:
                # write test accuracy to file "train_log"
                train_log_w = csv.writer(train_log)
                log_i = [int(epoch_n)] +sess.run([accuracy], feed_dict={inputs: dataset.test.images, outputs: dataset.test.labels, training: False})
                train_log_w.writerow(log_i)

    print("Final Accuracy: ", sess.run(accuracy, feed_dict={inputs: dataset.test.images, outputs: dataset.test.labels, training: False}), "%")
    sess.close()

def main(argv):
    try:
        opts, args = getopt.getopt(argv,'ht:',["train="])
